...
final class FrontGroupeProjetController extends AbstractController
{
    ...
    #[Route('/{id}', name: 'front_groupe_projet_show', methods: ['GET'])]
    ...
    #[Route('/{id}/edit', name: 'front_groupe_projet_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, GroupeProjet $groupeProjet, EntityManagerInterface $entityManager): Response
    {
        $user = $this->getUser();
        $isMember = $user && $groupeProjet->getIdUser()->contains($user);
        if (!$isMember && !$this->isGranted('ROLE_ADMIN')) {
            throw $this->createAccessDeniedException('Vous ne pouvez modifier que les groupes dont vous faites partie.');
        }
    ...
    #[Route('/{id}/delete', name: 'front_groupe_projet_delete', methods: ['POST'])]
    public function delete(Request $request, GroupeProjet $groupeProjet, EntityManagerInterface $entityManager): Response
    {
        $user = $this->getUser();
        $isMember = $user && $groupeProjet->getIdUser()->contains($user);
        if (!$isMember && !$this->isGranted('ROLE_ADMIN')) {
            throw $this->createAccessDeniedException('Vous ne pouvez supprimer que les groupes dont vous faites partie.');
        }
